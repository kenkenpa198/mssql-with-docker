...
- [2. WSL 環境の構築](#2-wsl-環境の構築)
    - [2.1. WSL のインストール](#21-wsl-のインストール)
    - [2.2. Ubuntu 環境の初期設定](#22-ubuntu-環境の初期設定)
    - [2.3. （オプション）WSL 環境のリセット](#23-オプションwsl-環境のリセット)
- [3. Docker のインストール](#3-docker-のインストール)
    - [3.1. Win 環境へ Docker Desktop をインストール](#31-win-環境へ-docker-desktop-をインストール)
    - [3.2. WSL 環境へ Docker をインストール](#32-wsl-環境へ-docker-をインストール)
    - [3.3. 補足: Docker Compose のインストールについて](#33-補足-docker-compose-のインストールについて)
- [4. キャッシュファイルの削除](#4-キャッシュファイルの削除)
- [5. Docker の実行テスト](#5-docker-の実行テスト)
- [6. 当リポジトリのクローン・初期設定](#6-当リポジトリのクローン初期設定)
- [8. （オプション）Azure Data Studio のインストール](#8-オプションazure-data-studio-のインストール)
- [9. 実行テスト](#9-実行テスト)
- [10. アーカイブ](#10-アーカイブ)
    - [10.1. （オプション）Docker を sudo 無しでも実行できるようにする](#101-オプションdocker-を-sudo-無しでも実行できるようにする)
...
この場合は [2.3. （オプション）WSL 環境のリセット](#23-オプションWSL-環境のリセット) に記載の手順を実行して環境を初期化する。  
初期化後、工程 1 からやり直す。

### 2.3. （オプション）WSL 環境のリセット
...
この工程では WSL 上での Docker の実行環境を整え、MSSQL のコンテナを立ち上げられるようにする。
...
### 3.3. 補足: Docker Compose のインストールについて
...
以上で WSL 環境と Docker の準備は完了。  
続けて当リポジトリ `mssql-with-docker` のクローン・初期設定を行う。

## 6. 当リポジトリのクローン・初期設定

当リポジトリをクローン後、`.env` ファイルを作成してパスワードを記述する。  
コンテナの立ち上げ時にこのファイルへ記述されたパスワードが環境変数として `docker-compose.yml` へ渡される。
...
### 10.1. （オプション）Docker を sudo 無しでも実行できるようにする

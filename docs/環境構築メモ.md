...
なお、WSL には大きく旧バージョンの「WSL 1」と新バージョンの「WSL 2」の2種類が存在する。  
当環境構築メモや README 上では WSL 2 をインストールするため、以降の名称も「WSL」で統一する。
...
※ [3.2. WSL 環境へ Docker をインストール](#32-WSL-環境へ-docker-をインストール) を対応していた場合、「Install Docker Engine」項でインストール済み。  
起動テストのみを行う。
